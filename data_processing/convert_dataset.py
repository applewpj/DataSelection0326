...
def convert_dolly_data(data_dir, output_dir, num_examples=None):
    os.makedirs(output_dir, exist_ok=True)
    examples = []
    ...
    if num_examples:
        examples = random.sample(examples, k=num_examples)
        output_path = os.path.join(output_dir, f"dolly_{num_examples}.jsonl")
    else:
        output_path = os.path.join(output_dir, "dolly_full.jsonl")
...
def convert_unnatural_instructions_data(data_dir, output_dir, num_examples=None):
    ...
    if num_examples:
        examples = random.sample(examples, k=num_examples)
        output_path = os.path.join(output_dir, f"unnatural_instructions_{num_examples}.jsonl")
    else:
        output_path = os.path.join(output_dir, "unnatural_instructions_full.jsonl")
    with open(output_path, "w") as fout:
        for idx, example in enumerate(examples):
            fout.write(
                json.dumps(
...
def convert_stanford_alpaca_data(data_dir, output_dir, num_examples=None):
    ...
    if num_examples:
        examples = random.sample(examples, k=num_examples)
        output_path = os.path.join(output_dir, f"stanford_alpaca_{num_examples}.jsonl")
    else:
        output_path = os.path.join(output_dir, "stanford_alpaca_full.jsonl")
    with open(output_path, "w") as fout:
        for idx, example in enumerate(examples):
            encoded_example = encode_instruction_example(
                instruction=example["instruction"],
...
def convert_med_alpaca_data(data_dir, output_dir, num_examples=None):
    os.makedirs(output_dir, exist_ok=True)
    examples = []
    with open(os.path.join(data_dir, "medical_meadow_wikidoc_medical_flashcards.json"), "r") as fin:
        examples.extend(json.load(fin))
    if num_examples:
        examples = random.sample(examples, k=num_examples)
    output_path = os.path.join(output_dir, "med_alpaca_data.jsonl")
    with open(output_path, "w") as fout:
        for idx, example in enumerate(examples):
            encoded_example = encode_instruction_example(
                instruction=example["instruction"],
...
def convert_code_alpaca_data(data_dir, output_dir, num_examples=None):
    ...
    if num_examples:
        examples = random.sample(examples, k=num_examples)
        output_path = os.path.join(output_dir, f"code_alpaca_{num_examples}.jsonl")
    else:
        output_path = os.path.join(output_dir, "code_alpaca_full.jsonl")
    with open(output_path, "w") as fout:
        for idx, example in enumerate(examples):
            encoded_example = encode_instruction_example(
                instruction=example["instruction"],
...
def convert_gpt4_alpaca_data(data_dir, output_dir, load_en=True, load_zh=False, num_examples=None):
    ...
    if num_examples:
        examples = random.sample(examples, k=num_examples)
        output_path = os.path.join(output_dir, f"gpt4_alpaca_{num_examples}.jsonl")
    else:
        output_path = os.path.join(output_dir, "gpt4_alpaca_full.jsonl")
    with open(output_path, "w") as fout:
        for idx, example in enumerate(examples):
            encoded_example = encode_instruction_example(
                instruction=example["instruction"],
...
def convert_sharegpt_data(
    ...
    if num_examples:
        examples = random.sample(examples, k=num_examples)
        output_path = os.path.join(output_dir, f"sharegpt_{num_examples}.jsonl")
    else:
        output_path = os.path.join(output_dir, "sharegpt_full.jsonl")
    with open(output_path, "w") as fout:
        invalid_cnt = 0
        for idx, example in enumerate(examples):
            messages = []
...
def convert_lima_data(data_dir, output_dir, num_examples=None):
    ...
    if num_examples:
        examples = random.sample(examples, k=num_examples)
        output_path = os.path.join(output_dir, f"lima_{num_examples}.jsonl")
    else:
        output_path = os.path.join(output_dir, "lima_full.jsonl")
    with open(output_path, "w") as fout:
        for idx, example in enumerate(examples):
            messages = []
...
def convert_wizardlm_data(data_dir, output_dir, num_examples=None):
    os.makedirs(output_dir, exist_ok=True)
    examples = []
    ...
    if num_examples:
        examples = random.sample(examples, k=num_examples)
        output_path = os.path.join(output_dir, f"wizardlm_{num_examples}.jsonl")
    else:
        output_path = os.path.join(output_dir, "wizardlm_full.jsonl")
    id_count = {}
    with open(output_path, "w") as fout:
        for idx, example in enumerate(examples):
            messages = []
            ...
            sample_id = f"wizardlm_{example['idx']}"
            if sample_id in id_count.keys():
                # pdb.set_trace()
                id_count[sample_id] += 1
            else:
                id_count[sample_id] = 1
            if id_count[sample_id] > 1:
                sample_id = sample_id + "_" + str(id_count[sample_id])
            fout.write(
                json.dumps(
                    {
                        "dataset": "wizardlm",
...
def convert_medinstruct_data(data_dir, output_dir, num_examples=None):
    os.makedirs(output_dir, exist_ok=True)
    examples = []
    with open(os.path.join(data_dir, "MedInstruct-52k.json"), "r") as fin:
        examples.extend(json.load(fin))
    if num_examples:
        examples = random.sample(examples, k=num_examples)
        output_path = os.path.join(output_dir, f"medinstruct_{num_examples}.jsonl")
    else:
        output_path = os.path.join(output_dir, "medinstruct_full.jsonl")
    with open(output_path, "w") as fout:
        for idx, example in enumerate(examples):
            encoded_example = encode_instruction_example(
                instruction=example["instruction"],
...
def convert_mathinstruct_data(data_dir, output_dir, num_examples=None):
    os.makedirs(output_dir, exist_ok=True)
    examples = []
    with open(os.path.join(data_dir, "MathInstruct.json"), "r") as fin:
        examples.extend(json.load(fin))
    if num_examples:
        examples = random.sample(examples, k=num_examples)
        output_path = os.path.join(output_dir, f"mathinstruct_{num_examples}.jsonl")
    else:
        output_path = os.path.join(output_dir, "mathinstruct_full.jsonl")
    with open(output_path, "w") as fout:
        for idx, example in enumerate(examples):
            encoded_example = encode_instruction_example(
                instruction=example["instruction"],
...
def convert_icliniq_data(data_dir, output_dir, num_examples=None):
    os.makedirs(output_dir, exist_ok=True)
    import pandas as pd
    text_csv = pd.read_csv(os.path.join(data_dir, "chatdoctor_text.csv"))
    examples = []
    for idx in range(len(text_csv)):
        raw_text = text_csv["text"][idx]
        prefix, instruction, input_text, output_text = raw_text.split("### ")
        instruction = instruction.strip().strip("Instruction:").strip()
        input_text = input_text.strip().strip("Input:").strip()
        output_text = output_text.strip().strip("Output:").strip()
        example = {"instruction": instruction, "input": input_text, "output": output_text}
        examples.append(example)
    
    # with open(os.path.join(data_dir, "chatdoctor_text.json"), "r") as fin:
    #     examples.extend(json.load(fin))
    if num_examples:
        examples = random.sample(examples, k=num_examples)
        output_path = os.path.join(output_dir, f"icliniq_{num_examples}.jsonl")
    else:
        output_path = os.path.join(output_dir, "icliniq_full.jsonl")
    with open(output_path, "w") as fout:
        for idx, example in enumerate(examples):
            encoded_example = encode_instruction_example(
                instruction=example["instruction"],
...
def convert_metamathqa_data(data_dir, output_dir, num_examples=None):
    ...
    if num_examples:
        examples = random.sample(examples, k=num_examples)
        output_path = os.path.join(output_dir, f"metamathqa_{num_examples}.jsonl")
    else:
        output_path = os.path.join(output_dir, "metamathqa_full.jsonl")
    with open(output_path, "w") as fout:
        for idx, example in enumerate(examples):
            encoded_example = encode_instruction_example(
...
def convert_magiccoder_data(data_dir, output_dir, num_examples=None):
    os.makedirs(output_dir, exist_ok=True)
    examples = []
    with open(os.path.join(data_dir, "data-evol_instruct-decontaminated.jsonl"), "r") as fin:
        # examples.extend(json.load(fin))
        for line in fin:
            examples.append(json.loads(line))
    if num_examples:
        examples = random.sample(examples, k=num_examples)
        output_path = os.path.join(output_dir, f"magiccoder_{num_examples}.jsonl")
    else:
        output_path = os.path.join(output_dir, "magiccoder_full.jsonl")
    with open(output_path, "w") as fout:
        for idx, example in enumerate(examples):
            encoded_example = encode_instruction_example(
                instruction=example["instruction"],
...
def convert_instinwild_data(data_dir, output_dir, num_examples=None):
    os.makedirs(output_dir, exist_ok=True)
    examples = []
    with open(os.path.join(data_dir, "instinwild_en.json"), "r") as fin:
        examples = json.load(fin)
    if num_examples:
        examples = random.sample(examples, k=num_examples)
        output_path = os.path.join(output_dir, f"instinwild_{num_examples}.jsonl")
    else:
        output_path = os.path.join(output_dir, "instinwild_full.jsonl")
    with open(output_path, "w") as fout:
        for idx, example in enumerate(examples):
            encoded_example = encode_instruction_example(
                instruction=example["instruction"],
...
def convert_medci_data(data_dir, output_dir, num_examples=None):
    os.makedirs(output_dir, exist_ok=True)
    examples = []
    # proceed as normally
    with open(os.path.join(data_dir, "release.json"), "r") as fin:
        examples = json.load(fin)
    # for idx, example in enumerate(examples):
    #     output_text = example["output"]
    #     output_text = output_text.strip().strip("###Answer:").strip()
    if num_examples:
        examples = random.sample(examples, k=num_examples)
        output_path = os.path.join(output_dir, f"medci_{num_examples}.jsonl")
    else:
        output_path = os.path.join(output_dir, "medci_full.jsonl")
    with open(output_path, "w") as fout:
        for idx, example in enumerate(examples):
            encoded_example = encode_instruction_example(
                instruction=example["instruction"],
...
def convert_fingpt_sentiment_data(data_dir, output_dir, num_examples=None):
    os.makedirs(output_dir, exist_ok=True)
    examples = []
    # proceed as normally
    with open(os.path.join(data_dir, "sentiment.jsonl"), "r") as fin:
        for line in fin:
            examples.append(json.loads(line))
    # for idx, example in enumerate(examples):
    #     output_text = example["output"]
    ...
    if num_examples:
        examples = random.sample(examples, k=num_examples)
        output_path = os.path.join(output_dir, f"fingpt_sentiment_{num_examples}.jsonl")
    else:
        output_path = os.path.join(output_dir, "fingpt_sentiment_full.jsonl")
    with open(output_path, "w") as fout:
        for idx, example in enumerate(examples):
            encoded_example = encode_instruction_example(
...
def convert_pqaa_data(data_dir, output_dir, num_examples=None):
    os.makedirs(output_dir, exist_ok=True)
    examples = []
    # proceed as normally
    with open(os.path.join(data_dir, "ori_pqaa.json"), "r") as fin:
        example_dict = json.load(fin)
        # pdb.set_trace()
        # examples = list(json.load(fin).values())
    ...
    if num_examples:
        examples = random.sample(examples, k=num_examples)
        output_path = os.path.join(output_dir, f"pqaa_{num_examples}.jsonl")
    else:
        output_path = os.path.join(output_dir, "pqaa_full.jsonl")
    with open(output_path, "w") as fout:
        for idx, example in enumerate(examples):
            encoded_example = encode_instruction_example(
...
def convert_lawinstruct_en_data(data_dir, output_dir, num_examples=None):
    os.makedirs(output_dir, exist_ok=True)
    ...
    #     filename = dataset + "-train-0.jsonl.xz"
    #     os.system(f"wget -P {data_dir} https://hf-mirror.com/datasets/lawinstruct/lawinstruct/resolve/main/data/{filename}")
    #     os.system(f"xz -d {os.path.join(data_dir, filename)}")
    examples = []
    for dataset in dataset_list:
        filename = dataset + "-train-0.jsonl"
        with open(os.path.join(data_dir, filename), "r") as f:
            for line in f:
                sample = json.loads(line)
                # if all(language == "en" for language in [sample["instruction_language"], sample["prompt_language"], sample["answer_language"]]):
                examples.append({
    ...
    if num_examples:
        examples = random.sample(examples, k=num_examples)
        output_path = os.path.join(output_dir, f"lawinstruct_en_{num_examples}.jsonl")
    else:
        output_path = os.path.join(output_dir, "lawinstruct_en_full.jsonl")
    with open(output_path, "w") as fout:
        for idx, example in enumerate(examples):
            encoded_example = encode_instruction_example(
                instruction=example["instruction"],
            ...
            fout.write(
                json.dumps(
                    {
                        "dataset": examples[idx]["dataset"],
                        # "subset": examples[idx]["subset"],
                        "id": f"{idx}",
                        "messages": [
                            {"role": "user", "content": encoded_example["prompt"]},
                            {"role": "assistant", "content": encoded_example["completion"]},
...
def convert_mmlu_data(data_dir, output_dir, num_examples=None):
    os.makedirs(output_dir, exist_ok=True)
    examples = []
    import pyarrow.parquet as pq
    source_parquet_path = os.path.join(data_dir, 'dev-00000-of-00001.parquet')
    table = pq.read_table(source_parquet_path)
    df = table.to_pandas()
    source_json_path = os.path.join(data_dir, 'dev.jsonl')
    ...
    # output_path = os.path.join(output_dir, "medci_data.jsonl")
    # with open(output_path, "w") as fout:
    #     for idx, example in enumerate(examples):
    #         encoded_example = encode_instruction_example(
    #             instruction=example["instruction"],
...
def convert_ultra_data(data_dir, output_dir, num_examples=None):
    os.makedirs(output_dir, exist_ok=True)
    examples = []
    # with open(os.path.join(data_dir, "ultra_mcqa.json"), "r") as fin:
    with open("/mnt/hwfile/medai/LLMModels/MedicalDatas/train/ultra_mcqa.json", "r") as fin:
        examples.extend(json.load(fin))
    if num_examples:
        examples = random.sample(examples, k=num_examples)
        output_path = os.path.join(output_dir, f"ultra_{num_examples}.jsonl")
    else:
        output_path = os.path.join(output_dir, "ultra_full.jsonl")
    with open(output_path, "w") as fout:
        for idx, example in enumerate(examples):
            encoded_example = encode_instruction_example(
                instruction=example["input"],
...
if __name__ == "__main__":
    ...
    for dataset in args.dataset:
        ...
        globals()[f"convert_{dataset}_data"](
            os.path.join(args.raw_data_dir, dataset), os.path.join(args.output_dir, dataset), num_examples=args.num_examples
        )
        # convert_dolly_data(
        #         data_dir=os.path.join(args.raw_data_dir, "dolly"),
        #         output_dir=os.path.join(args.output_dir, "tulu_v1", "dolly_subset"),
